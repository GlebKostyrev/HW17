...
class MoviesView(Resource):
    ...
    def get(self):
        movies_quere = db.session.query(models.Movie)

        args = request.args

        director_id = args.get('director_id')
        if director_id is not None:
            movies_quere = movies_quere.filter(models.Movie.director_id == director_id)

        genre_id = args.get('genre_id')
        if genre_id is not None:
            movies_quere = movies_quere.filter(models.Movie.genre_id == genre_id)

        movies = movies_quere.all()
